...
public class TestSetUpGit {
	...
	// new MockComponentWorker().init().addMock(ProjectManager.class,
	// projectManager);
	//
	...
	//
	// File file = new File(
	// System.getProperty("user.home") + File.separator + "repository" +
	// File.separator + projectKey);
	// if (file.exists()) {
	...
	// cloneDir.mkdirs();
	// git =
	// Git.cloneRepository().setURI(remoteRepo.getDirectory().getAbsolutePath()).setDirectory(cloneDir)
	...
	// "refs/heads/master");
	// config.setString(ConfigConstants.CONFIG_BRANCH_SECTION, "Branch", "remote",
	// "origin");
	// config.setString(ConfigConstants.CONFIG_BRANCH_SECTION, "Branch", "merge",
	// "refs/heads/Branch");
	// config.save();
	...
	// git.commit().setMessage("FirstFile").setAuthor("gildas",
	// "gildas@example.com").call();
	//
	...
	// TestComponentGetter.init(new TestActiveObjects(entityManager), new
