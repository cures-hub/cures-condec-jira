...
public class TestBoostIfDecisionProblem extends TestSetUp {
    ...
    @Test
	public void testPropagationNonDecisionProblemMinRuleWeight() {
		currentElement = KnowledgeElements.getProArgument();
        List<ChangePropagationRule> propagationRules = new LinkedList<>();
		propagationRules.add(new ChangePropagationRule("BOOST_IF_DECISION_PROBLEM", false, 0.0f));
		...
		assertEquals(1.0, ChangePropagationRuleType.BOOST_IF_DECISION_PROBLEM.getFunction()
				.isChangePropagated(filterSettings, currentElement, null), 0.005);
	}

    @Test
	public void testPropagationDecisionProblemMaxRuleWeight() {
		currentElement = KnowledgeElements.getUnsolvedDecisionProblem();
        List<ChangePropagationRule> propagationRules = new LinkedList<>();
		propagationRules.add(new ChangePropagationRule("BOOST_IF_DECISION_PROBLEM", false, 2.0f));
		ChangeImpactAnalysisConfiguration config = new ChangeImpactAnalysisConfiguration(0.25f, 0.25f, (long) 0,
				propagationRules);
		ConfigPersistenceManager.saveChangeImpactAnalysisConfiguration("TEST", config);
		filterSettings = new FilterSettings("TEST", "");
		...
		assertEquals(0.0, ChangePropagationRuleType.BOOST_IF_DECISION_PROBLEM.getFunction()
				.isChangePropagated(filterSettings, currentElement, null), 0.005);
	}
}