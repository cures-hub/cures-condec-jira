...
public class TestSetStatus extends TestSetUp {
	...
	private final static String BAD_REQUEST_ERROR = "Setting the element status failed due to a bad request.";
	...
	public void testNullFilledNull() {
		assertEquals(Response.status(Response.Status.BAD_REQUEST).entity(ImmutableMap.of("error", BAD_REQUEST_ERROR))
				.build().getEntity(), knowledgeRest.setStatus(null, KnowledgeStatus.IDEA.toString(), null).getEntity());
	}
	...
	public void testNullFilledFilled() {
		assertEquals(
				Response.status(Response.Status.BAD_REQUEST).entity(ImmutableMap.of("error", BAD_REQUEST_ERROR)).build()
						.getEntity(),
				knowledgeRest.setStatus(null, KnowledgeStatus.IDEA.toString(), decisionKnowledgeElement).getEntity());
	}
	...
	public void testFilledNullFilled() {
		assertEquals(Response.status(Response.Status.BAD_REQUEST).entity(ImmutableMap.of("error", BAD_REQUEST_ERROR))
				.build().getEntity(), knowledgeRest.setStatus(request, null, decisionKnowledgeElement).getEntity());
	}
	...
	public void testFilledFilledNull() {
		assertEquals(
				Response.status(Response.Status.BAD_REQUEST).entity(ImmutableMap.of("error", BAD_REQUEST_ERROR)).build()
						.getEntity(),
				knowledgeRest.setStatus(request, KnowledgeStatus.IDEA.toString(), null).getEntity());
	}

	@Test
	@NonTransactional
	public void testFilledFilledFilled() {
		assertEquals(Response.Status.OK.getStatusCode(), knowledgeRest
				.setStatus(request, KnowledgeStatus.IDEA.toString(), decisionKnowledgeElement).getStatus());
	}
}
