...
public class TestGetDuplicatesForIssue extends TestConsistencyRestSuper {
	...
	public void testGetDuplicatesForIssue() {
		...
		Response response = consistencyRest.getDuplicatesKnowledgeElements(request, knowledgeElement.getProject().getProjectKey(), knowledgeElement.getId(), knowledgeElement.getDocumentationLocationAsString());
		...
		response = consistencyRest.getDuplicatesKnowledgeElements(request, knowledgeElement.getProject().getProjectKey(), knowledgeElement.getId(), knowledgeElement.getDocumentationLocationAsString());
		assertEquals("Request should be valid.", 200, response.getStatus());

		issues.get(1).setDescription(oldDescription);
		response = consistencyRest.getDuplicatesKnowledgeElements(request, "null", knowledgeElement.getId(), knowledgeElement.getDocumentationLocationAsString());
		assertEquals("Request should be invalid.", 400, response.getStatus());
		response = consistencyRest.getDuplicatesKnowledgeElements(request, knowledgeElement.getProject().getProjectKey(), -1L, knowledgeElement.getDocumentationLocationAsString());
		assertEquals("Request should be invalid.", 400, response.getStatus());
		response = consistencyRest.getDuplicatesKnowledgeElements(request, null, knowledgeElement.getId(), knowledgeElement.getDocumentationLocationAsString());
		assertEquals("Request should be invalid.", 400, response.getStatus());
	}
