...
public class TestGetRefs extends TestSetUpGit {
	...
	public void testDefaultBranch() {
		assertEquals("refs/remotes/origin/master", gitClient.getRefs("master").get(0).getName());
	}

	@Test
	public void testFeatureBranch() {
		assertEquals("refs/remotes/origin/TEST-4.feature.branch",
				gitClient.getRefs("feature.branch").get(0).getName());
	}

	@Test
	public void testJiraIssueKey() {
		assertEquals("refs/remotes/origin/TEST-4.feature.branch", gitClient.getRefs("TEST-4").get(0).getName());
	}

	@Test
	public void testBranchNameInvalid() {
		assertEquals(0, gitClient.getRefs("").size());
		assertEquals(0, gitClient.getRefs(null).size());
	}
