...
public class testNumberOfCommentsPerJiraIssue extends TestSetUpGit {
	...
	public void testRequestNull() {
		...
		assertEquals(Status.BAD_REQUEST.getStatusCode(),
			dashboardRest.getNumberOfCommentsPerJiraIssue(null, null).getStatus());
	}
	...
	public void testIssueKeyNull() {
		ConfigPersistenceManager.setKnowledgeExtractedFromGit("", true);
		assertEquals(Status.BAD_REQUEST.getStatusCode(),
			dashboardRest.getNumberOfCommentsPerJiraIssue(request, null).getStatus());
	}
	...
	public void testEmptyIssueKey() {
		ConfigPersistenceManager.setKnowledgeExtractedFromGit("", true);
		assertEquals(Status.BAD_REQUEST.getStatusCode(),
			dashboardRest.getNumberOfCommentsPerJiraIssue(request, "").getStatus());
	}

	@Test
	public void testUnknownIssueKey() {
		assertEquals(Status.BAD_REQUEST.getStatusCode(),
			dashboardRest.getNumberOfCommentsPerJiraIssue(request, "HOUDINI-1").getStatus());
	}
	...
	public void testGitExtractionDisabled() {
		...
		assertEquals(Status.SERVICE_UNAVAILABLE.getStatusCode(),
			dashboardRest.getNumberOfCommentsPerJiraIssue(request, "TEST-2").getStatus());
