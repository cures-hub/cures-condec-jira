...
public class GitCommitMessageExtractor {
    ...
    private void classifyMessage() {
        ...
        List<PartOfText> parts = splitter.getPartsOfText(this.fullMessage, this.projectKey);

        StringBuilder builder = new StringBuilder();

        //Get only the text of the part of text list
        List<String> partStrings = parts.stream().map(PartOfText::getText).collect(Collectors.toList());

        // Classification
        List<Boolean> isRelevantPredictions = decisionKnowledgeClassifier.makeBinaryPredictions(partStrings);

        //create separate list of all relevant parts for fine grained prediction
        List<String> relevantMessageParts = IntStream
                .range(0, isRelevantPredictions.size())
                .filter(isRelevantPredictions::get)
                .mapToObj(partStrings::get)
                .collect(Collectors.toList());


        List<KnowledgeType> fineGrainedPredictions = decisionKnowledgeClassifier.makeFineGrainedPredictions(relevantMessageParts);

        // build comment
        for (int partsIndex = 0, fineGrainedPredictionIndex = 0; partsIndex < parts.size(); partsIndex++) {
            if (isRelevantPredictions.get(partsIndex)) {
                // if a part is relevant the tags are added
                KnowledgeType prediction = fineGrainedPredictions.get(fineGrainedPredictionIndex);
                String predicitionTag = "{" + prediction.name().toLowerCase() + "}";
                builder.append(predicitionTag);
                builder.append(partStrings.get(partsIndex));
                builder.append(predicitionTag);
                fineGrainedPredictionIndex++;
            } else {
