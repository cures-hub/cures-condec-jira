...
public class Colorizer {
	...
	public static TreeViewerNode colorizeTreeNode(List<KnowledgeElementWithImpact> impactedElements,
			TreeViewerNode node, FilterSettings filterSettings) {
		String style = "";
		...
		for (KnowledgeElementWithImpact element : impactedElements) {
			if (node.getElement().getId() == element.getId()) {
				...
				node.setLiAttr(ImmutableMap.<String, String>builder().put("style", style).put("class", clzz)
						.put("cia_tooltip", Tooltip.createTooltip(element, filterSettings)).build());
		...
		if (!node.getLiAttr().containsKey("cia_tooltip")) {
			node.setLiAttr(ImmutableMap.<String, String>builder()
					.put("style", "background-color:" + recommendationColor).put("class", node.getLiAttr().get("class"))
					.put("cia_tooltip", Tooltip.createLinkRecommendationTooltip()).build());
	...
	public static VisNode colorizeVisNode(List<KnowledgeElementWithImpact> impactedElements, VisNode node,
	...
	public static MatrixNode colorizeMatrixNode(List<KnowledgeElementWithImpact> impactedElements, MatrixNode node,
			FilterSettings filterSettings) {
		for (KnowledgeElementWithImpact element : impactedElements) {
			if (node.getElement().getId() == element.getId()) {
				// Painting the background color white for the root node to prevent a red
		...
		// Colorization for link recommendation nodes that have no CIA score
		if (!node.getChangeImpactExplanation().contains("CIA")
				&& !node.getChangeImpactExplanation().contains("Change Impact Analysis")) {
	...
	public static String colorForImpact(double impact) {
		Color red = new Color(213, 99, 23); // #C79E73, https://www.colorhexa.com/d56317
