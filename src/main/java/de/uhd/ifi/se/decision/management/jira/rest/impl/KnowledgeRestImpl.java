...
public class KnowledgeRestImpl implements KnowledgeRest {
	...
	public Response assignDecisionGroup(@Context HttpServletRequest request, @QueryParam("sourceId") long sourceId,
		...
		/*
		 * if (decisionKnowledgeElement == null || request == null) { return
		 * Response.status(Status.BAD_REQUEST) .entity(ImmutableMap.of("error",
		 * "Deletion of decision knowledge element failed.")).build(); } String
		 * projectKey = decisionKnowledgeElement.getProject().getProjectKey();
		 * ApplicationUser user = AuthenticationManager.getUser(request);
		 * 
		 * boolean isDeleted = KnowledgePersistenceManager.getOrCreate(projectKey)
		 * .deleteDecisionKnowledgeElement(decisionKnowledgeElement, user); if
		 * (isDeleted) { return Response.status(Status.OK).entity(true).build(); }
		 * return Response.status(Status.INTERNAL_SERVER_ERROR)
		 * .entity(ImmutableMap.of("error",
		 * "Deletion of decision knowledge element failed.")).build();
		 */
	}
