...
/**
 * Class to compute the metrics for the proposals and compare the ratings
 */
public class ReleaseNotesCreator {
	...
	public ReleaseNotesCreator(List<DecisionKnowledgeElement> elementsMatchingQuery, ReleaseNoteConfiguration releaseNoteConfiguration, ApplicationUser user) {
		...
		proposals.forEach(dkElement -> {
			...
			criteriaEnumList.forEach(criteria -> {
				...
				if (criteria == JiraIssueMetric.PRIORITY) {
					...
					scaling = RatingCalculator.scaleFromSmallToLarge(existingCriteriaValues.get(criteria), minValues.get(criteria).get(indexMinPrio), maxValues.get(criteria).get(indexMaxPrio), 1, 10);
					scaling = 11 - scaling;
				} else {
