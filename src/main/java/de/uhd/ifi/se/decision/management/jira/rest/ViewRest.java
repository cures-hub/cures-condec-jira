...
public class ViewRest {
	...
	@GET
	@Produces({MediaType.APPLICATION_JSON})
	public Response getRecommendationEvaluation(@Context HttpServletRequest request, @QueryParam("projectKey") String projectKey,
												@QueryParam("keyword") String keyword, @QueryParam("knowledgeSource") String knowledgeSourceName) {
		if (request == null || projectKey == null) {
			return Response.status(Status.BAD_REQUEST).entity(ImmutableMap.of("error", "Project Key is not correct!"))
				.build();
		}
		...
		List<Recommendation> recommendationList = simpleRecommender
			.addKnowledgeSourceForEvaluation(allKnowledgeSources, knowledgeSourceName)
			.evaluate();
