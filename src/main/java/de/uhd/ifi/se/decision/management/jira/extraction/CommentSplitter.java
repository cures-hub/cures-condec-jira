...
public interface CommentSplitter {
	...
	String[] RATIONALE_TAGS = new String[] { "{issue}", "{alternative}", "{decision}", "{pro}", "{con}", "{codesummarization}" };

	/** List of all knowledge types as icons. Sequence matters! */
	String[] RATIONALE_ICONS = new String[] { "(!)", "(?)", "(/)", "(y)", "(n)", "(i)" };
	...
	Set<KnowledgeType> KNOWLEDGE_TYPES = EnumSet.of(KnowledgeType.DECISION, KnowledgeType.ISSUE, KnowledgeType.PRO,
			KnowledgeType.CON, KnowledgeType.ALTERNATIVE,  KnowledgeType.CODESUMMARIZATION);
