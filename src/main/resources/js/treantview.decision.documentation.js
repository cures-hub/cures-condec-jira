function buildTreant(projectKey, node) {
	...
	var treantUrl = AJS.contextPath()
			+ "/rest/treantsrest/latest/treant.json?projectKey=" + projectKey
			+ "&elementKey=" + node.key + "&depthOfTree=" + depthOfTree;
	getJSON(
			treantUrl,
			function(error, treant) {
				if (error != null) {
					...
					var elementsWithCloseFunction = document
							.getElementsByClassName("modal-close");
					...
					items : {
						"add" : {
							...
							callback : function(key, options) {
								...
								for (var index = 0; index < knowledgeTypes.length; index++) {
									type_select[0].insertAdjacentHTML(
											'beforeend', '<option value="'
								...
								setSubmitFunction(submitButton, type_select,
										projectKey, options.$trigger.context.id);

								var modal = document
										.getElementById('ContextMenuModal');
							...
							callback : function(key, options) {
								...
								for (var index = 0; index < children.length; index++) {
									if (children.hasOwnProperty(index)) {
										if (index === 0) {
											document
													.getElementById('form-input-name').value = children[index].innerText;
										} else if (index == 1) {
								...
								submitButton.onclick = function() {
									...
									editDecisionKnowledgeElement(context.id,
						...
						"delete" : {
							...
							callback : function(key, options) {
								...
								var abortButton = document
										.getElementById('abort-submit');
								...
								var submitButton = document
										.getElementById('form-input-submit');
								submitButton.onclick = function() {
									deleteDecisionKnowledgeElement(context.id,
											function() {
												var nodeId = $.jstree
														.reference('#evts')
														.get_selected()[0];
