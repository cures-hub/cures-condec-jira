function linkBranchCandidates(rationale, branchName, branchInSequence, problemSourceCategory) {

	// bad problem explanations
	var ARGUMENT_WITHOUT_PARENT_ELEMENT = "Argument without parent alternative"
	var ALTERNATIVE_DECISION_WITHOUT_PARENT_ELEMENT = "Alternative without parent problem"
	...
	var ALTERNATIVE_DECISION_WITHOUT_ARGUMENTS = "Alternative does not have any arguments"
	var DECISION_ARGUMENTS_MAYBE_WORSE_THAN_ALTERNATIVE = "Decision's arguments seem weaker than in one of sibling alternatives"
	...
	function runCodelinker() {
		...
		function attachProblemsToElementsInHTML() {
			...
			for (var i = 0; i< problemsList.length; i++) {
				...
				if ( problemWithRationale.problems.warnings.length>0) {
					...
					problemElement.title += problemElement.title
					problemElement.innerText += problemElement.title
						+ " "
						+ problemWithRationale.problems.warnings.toString()
					for (var w = 0; w<problemWithRationale.problems.warnings.length; w++) {
				...
				if ( problemWithRationale.problems.errors.length>0) {
					...
					problemElement.innerText += problemElement.title
						+ " "
						+ problemWithRationale.problems.errors.toString()

					for (var e = 0; e<problemWithRationale.problems.errors.length; e++) {
		...
		function appendProblemCategoryToQualitySummary(qElem,categorySet, prefix) {
			...
			catElem.innerText = prefix + problemSourceCategory + ": " + problemsAsList
		...
		*/
		function composeId(problemStructure) {
			...
			var htmlID = arrayEllement.rationaleHash
				+ "-"
				+ branchName
				+ "-"
		...
		function addNewAlternative(el, isDecision) {
			...
			if (canBeLinkedWithParent(el)) {
				if (linkCandidates[currentIssueId]
			...
			el.getElementProblems = function() {
				if (!this.hasParent()) {
					this.problems.errors.push(ALTERNATIVE_DECISION_WITHOUT_PARENT_ELEMENT)
				}
				...
				if (this.isDecisionOutvotedByAlternative()) {
					this.problems.warnings.push(DECISION_ARGUMENTS_MAYBE_WORSE_THAN_ALTERNATIVE)
				}
		...
		function addNewArgument(el, isProArg) {
			...
			if (canBeLinkedWithParent(el)) {
				if (linkCandidates[currentAlternativeId]
		...
		function canBeLinkedWithParent(element) {
			...
			if (element.positionEndLine &&
				(nextRationaleExpectedOnLine === null || nextRationaleExpectedOnLine < element.positionStartLine)
			 ) {
				return false
			}
			return true;
