function buildTreeViewer(projectKey, nodeId) {
	var treeJSONUrl = AJS.contextPath()
			+ "/rest/treeviewerrest/latest/treeviewer.json?projectKey="
			+ projectKey;
	getJSON(
			treeJSONUrl,
			function(err, data) {
												...
												'edit' : {
													...
													'action' : function(node) {
														...
														content
																.insertAdjacentHTML(
																		'afterBegin',
																		'<p><label for="form-input-name" style="display:block;width:45%;float:left;">Summary:</label><input id="form-input-name" type="text" name="summary" value="'
																				+ tree_node.summary
