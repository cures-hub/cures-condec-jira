...
var RATIONALE_NO_CHANGES_TEXT =
  "{file} - no rationale changed in below section:";
...
function getElementAsHTML(element, isFromMessage) {
  ...
  if (isFromMessage) {
    ...
    locationText =
      "Code comment section at position (start line, end line, sequence # in comment) " +
      locationTextShort;
  }
...
function appendCodeElements(brNode) {
  ...
  while ((blockEntry = blockLinesIterator.next())) {
    ...
    if (blockIsDiffType) {
      ...
      fileRatBlockLabel.innerText = RATIONALE_NO_CHANGES_TEXT.replace(
        "{file}",
        blockData.filename
      );
  ...
  if (elementsFromMessage !== null && elementsFromMessage.length > 0) {
    ...
    for (m = 0; m < elementsFromMessage.length; m++) {
      if (msgCommitIsh !== elementsFromMessage[m].key.source) {
        ...
        messageBlockHtml.id =
          "branchGroup-" + lastBranchIdx + "-message-" + msgCommitIsh;
...
function appendBranchQualityAssessment(parentNode, index) {
  ...
  if (
    (lastBranchElementsFromMessages &&
      lastBranchElementsFromMessages.length > 0) ||
...
function showBranchesDiff(data) {
  ...
  if (data.branches.length > 0) {
    ...
    var messageLabels = contentHtml.getElementsByClassName(
      "commitMessageLabel"
    );
    var blockForNonDiffElements = contentHtml.getElementsByClassName(
      "fileNonDiffBlockLabel"
    );
    var blockForDiffElements = contentHtml.getElementsByClassName(
      "fileDiffBlockLabel"
    );
